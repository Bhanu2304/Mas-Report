...
if(!empty($_POST))
{
    ...
    $priority_list = $_POST['priority'];
    $report_id = $_POST['report_id'];
    //print_r($priority_list);exit;
    $ins_arr = array();
    foreach($column_list as $cl)
    {
        ...
        $priority = $priority_list["$tbl_id"."_".$column_name];
        $ins_arr[] = "('$report_id','$tbl_id','$column_name','$priority')";
    }
    
    $delete = "delete  from tbl_report_field where report_id='$report_id'";
    $Rscx_Del = mysqli_query($con,$delete);
    $ins_str = "insert into tbl_report_field(report_id,tbl_id,column_name,priority) values".implode(",",$ins_arr);   
    $Rscx = mysqli_query($con,$ins_str);
		...
		header("location:report-field-selection.php?msg=succ&report_id=".$report_id);
		$_SESSION['msg']= "Report Field Updated successfully";
                exit;
                
                }
		else
		{	
		header("location:report-field-selection.php?msg=unsucc&report_id=".$report_id);
                $_SESSION['msg']= "Report Field Updation Failed";
                exit;
		}
	}
	
        
$sel_tbl = "SELECT * FROM tbl_used WHERE id IN (SELECT tbl_id FROM `tbl_resultant` WHERE  active='1')";
...
while($tbl = mysqli_fetch_assoc($rsc_tbl))
{
    ...
    $sel_column = "SELECT * FROM `tbl_resultant` WHERE tbl_id='$tbl_id'";
...
$sel_rtbl = "SELECT tbl_id,column_name,priority FROM tbl_report_field where report_id='$report_id'";
$rsc_rtbl = mysqli_query($con,$sel_rtbl);
$report_column_list = array();
$report_priority_list = array();
while($col = mysqli_fetch_assoc($rsc_rtbl))
{
    $report_column_list[$col['tbl_id']][] =$col['column_name']; 
    $report_priority_list[$col['tbl_id']][$col['column_name']] =$col['priority']; 
}
...
//print_r($report_priority_list);exit;
?>
<!doctype html>
<html lang="en">
<head>
...
{
    window.location.href = 'report-field-selection.php?report_id='+report_id;
}
</script>
...
<div id="<?php echo $tbl_id;?>" class="w3-container tbl">
  ...
  <div>
      <table class="table table-hover">
          <thead>
              <tr>
                  <th>Priority</th><th>Column 1</th>
                  <th>Priority</th><th>Column 2</th>
                  <th>Priority</th><th>Column 3</th>
                  <th>Priority</th><th>Column 4</th>
              </tr>
          </thead>
          <tbody>
              <tr>
              ...
              foreach($tbl_column_list[$tbl_id] as $column_name) 
                  ...
                  <td><u><?php  $srno++; ?> <input style="width:50px;" form="form11" 
                    type="text" name="priority[<?php echo $key; ?>]" 
                    value="<?php echo $report_priority_list[$tbl_id][$column_name]; ?>" /></u> 
                  </td>
                  <td>    <?php echo $column_name;  ?>
